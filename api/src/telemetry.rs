...
/// Installs the JSON subscriber. An unparseable `RUST_LOG` is reported like
/// any other invalid environment variable; the subscriber is still installed
/// (with the `info` default) so the caller can log that error as JSON before
/// exiting.
