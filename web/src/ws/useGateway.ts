...
export function mediaContextTopics(input: {
  ...
  if (input.watching && input.watchServerId && input.watchChannelId) {
    topics.push({ s: input.watchServerId });
    topics.push({ s: input.watchServerId, c: input.watchChannelId });
  }
...
export function useAuthenticatedSubscriptions(): void {
  ...
  }, [routeServerId, routeChannelId, server, dms, voice]);
  ...
  const clearRememberedServer = useCallback((serverId: string) => {
    setRememberedServerId((current) =>
      current === serverId ? undefined : current,
    );
  }, []);
  useRealtimeBridge(routeServerId, clearRememberedServer);
}
