...
- **E-Mail + Passwort**, Hash **Argon2id** (19 MiB, t=2, p=1, PHC-String). Unbekannte E-Mail wird gegen einen Dummy-Hash geprüft, damit Login-Zeiten nichts verraten.
- **Session** = Cookie `gelabber_session` (`HttpOnly; SameSite=Lax; Path=/`, `Max-Age` aus `API_SESSION_TTL_HOURS`, Default 30 Tage; `Secure` per `API_COOKIE_SECURE=true`). In Postgres liegt nur der SHA-256 des Tokens.
...
- **Nicht in v1**: OAuth, fremdes JWT, Magic Links, 2FA, SSO, Passkeys, E2E. Avatar ist in v1 eine URL; Datei-Upload kommt mit dem Dateien-/MinIO-Ticket.
