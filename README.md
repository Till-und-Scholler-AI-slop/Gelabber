...
| `{"op":"sig","t":"p\|u","s":"…","c":"…","k":"a\|v\|s"}` | Pub / unpub. Audio (`a`), Kamera (`v`) und Screen (`s`) im Sprachkanal; Go Live ist Issue 14 |
...
| `GET /api/channels/{id}/messages` | `200 {messages, has_more}` — kanal-lokale History, oldest→newest; `before`/`after` = Message-ID oder RFC3339, `limit` 1–100 (Default 50). Voice / fremder Kanal → `404`. DMs nutzen dieselben Pfade |
| `POST /api/channels/{id}/messages` | `{content}` (1–2000 Zeichen) → `201 Message` (`send_messages`; in einer DM dürfen beide schreiben) |
| `PATCH /api/messages/{id}` | `{content}` → `200 Message` (nur Autor + `send_messages`) |
| `DELETE /api/messages/{id}` | `204` (nur Autor) |
...
- **Rechte-Flags** (`api/src/servers/permissions.rs`): `manage_server`, `manage_channels`, `send_messages`, `send_files`, `join_voice`, `go_live`. Pro Server **eine** Maske für alle Member (`member_permissions`, Default: alles außer verwalten); der Owner hat immer alles. Keine Channel-Overwrites. Handler prüfen mit `Membership::require(Permission::…)`. Text-Chat hängt an `send_messages`; Dateien, Voice und Live folgen in späteren Tickets.
...
- Fehler-Codes zusätzlich zu Auth: `forbidden`, `not_found`, `invite_invalid`. Feld-Codes wie bisher; neue Felder: `kind`, `category_id`, `member_permissions`, `max_uses`, `expires_in_hours`, `content`, `before`, `after`, `limit`, `user_id`.
...
- **Schreiben / Edit / Delete**: Textkanäle und 1:1-DMs. Lesen darf jedes Mitglied (bzw. beide DM-Teilnehmer); Posten und Editieren brauchen `send_messages` im Server, in einer DM dürfen beide schreiben; Löschen ist allein der Autor (kein Moderations-Delete in v1). Fremde oder Voice-Kanäle sind `404 not_found`. Inhalt: getrimmt, 1–2000 Zeichen, kein Steuerzeichen außer Tab/Newline.
...
- **Kein WS hier**: Live-Events sind Issue 6. sqlx bleibt 0.9.0.
...
Chat (issue 5): Textkanal-Seite virtualisiert die History (TanStack Virtual, dynamische Zeilenhöhe). Senden schreibt die Zeile zuerst in ein kanal-lokales Pending-Overlay (`tmp:`-ID) — sie steht im selben Frame, unter 50 ms, ohne auf GET/POST zu warten. Der Server ersetzt die ID; ein Fehler nimmt die Zeile wieder weg (Toast, keine Geisterzeile). Edit/Delete der eigenen Nachricht sind ebenfalls optimistisch. Scrollen nach oben lädt die nächste ältere Seite per ID-Cursor; am unteren Rand bleibt die Liste kleben, wenn man schon unten war. Voice (issue 12): Join/Leave, Mute/Deafen session-lokal (sofort, dann Sync), Voice-State in der Mitgliederliste mit reservierten Icon-Slots. `join_voice` steuert den Beitritt. Kein LiveKit, kein fremdes SDK.
