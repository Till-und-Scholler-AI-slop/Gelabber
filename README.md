...
| `GET /api/channels/{id}/messages` | `200 {messages, has_more}` — kanal-lokale History, oldest→newest; `before`/`after` = Message-ID oder RFC3339, `limit` 1–100 (Default 50). Voice / fremder Kanal → `404` |
| `POST /api/channels/{id}/messages` | `{content}` (1–2000 Zeichen) → `201 Message` (`send_messages`) |
...
- Fehler-Codes zusätzlich zu Auth: `forbidden`, `not_found`, `invite_invalid`. Feld-Codes wie bisher; neue Felder: `kind`, `category_id`, `member_permissions`, `max_uses`, `expires_in_hours`, `content`, `before`, `after`, `limit`.

### Textnachrichten (issue 5)

- **Schreiben / Edit / Delete**: Nur Textkanäle. Lesen darf jedes Mitglied; Posten und Editieren brauchen `send_messages`; Löschen ist allein der Autor (kein Moderations-Delete in v1). Fremde oder Voice-Kanäle sind `404 not_found`. Inhalt: getrimmt, 1–2000 Zeichen, kein Steuerzeichen außer Tab/Newline.
